...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        sms_template_objs = self.env["sms.template"].sudo().search(
            [('condition', '=', 'order_confirm'),('globally_access','=',False),('id','in',self.operating_unit_id.sms_template_id.ids)])
    ...
    def action_cancel(self):
        ...
        sms_template_objs = self.env["sms.template"].sudo().search(
            [('condition', '=', 'order_cancel'),('globally_access','=',False),('id','in',self.operating_unit_id.sms_template_id.ids)])
        for obj in self:
    ...
    def write(self, vals):
        ...
        for res in self:
            if res and vals.get("state", False) == 'sent':
                sms_template_objs = self.env["sms.template"].sudo().search(
                    [('condition', '=', 'order_placed'),('globally_access','=',False),('id','in',self.operating_unit_id.sms_template_id.ids)])
                for sms_template_obj in sms_template_objs:
...
class stock_picking(models.Model):
    ...
    def send_picking_done_message(self):
        sms_template_objs = self.env["sms.template"].sudo().search(
            [('condition', '=', 'order_delivered'),('globally_access','=',False),('id','in',self.picking_type_id.warehouse_id.operating_unit_id.sms_template_id.ids)])
        for sms_template_obj in sms_template_objs:
            mobile = sms_template_obj._get_partner_mobile(
                self.partner_id)
            if mobile:
...
class account_invoice(models.Model):
    ...
    def send_invoice_message(self,state):
        if state == 'open':
            ...
            sms_template_objs = self.env["sms.template"].sudo().search(
                [('condition', '=', 'invoice_vaildate'),('globally_access','=',False),('id','in',self.operating_unit_id.sms_template_id.ids)])
            for sms_template_obj in sms_template_objs:
                mobile = sms_template_obj._get_partner_mobile(
                    self.partner_id)
                if mobile:
